...
//				System.out.printf("totalContent=%s, totalPage=%s%n", totalContent, totalPage);
				...
				//1.이전
				if(pageNo != 1) {
					pageBar += "<a href='"+request.getContextPath()+"/admin/rentalList?cPage="+(pageNo-1)+"'>[이전]</a>\n";
				}
				...
				while(pageNo<=pageEnd && pageNo<=totalPage) {
					...
					if(cPage == pageNo) {
						...
						pageBar += "<a href='"+request.getContextPath()+"/admin/rentalList?cPage="+pageNo+"'>"+pageNo+"</a>\n";				
					}
				...
				//3.다음
				if(pageNo <= totalPage) {
					pageBar += "<a href='"+request.getContextPath()+"/admin/rentalList?cPage="+pageNo+"'>[다음]</a>\n";							
				}
