...
public class ItemRentEachDAO {
	...
	public ItemRentEach selectOne(Connection conn, String memberId) {
		...
		try{
			//미완성쿼리문을 가지고 객체생성.
			pstmt = conn.prepareStatement("SELECT * FROM item_rent_each WHERE item_rent_user=?");
			//쿼리문미완성
			...
			if(rset.next()){
				...
				ItemRentEach.setItemRentYN((rset.getString("board_title")).charAt(0));
